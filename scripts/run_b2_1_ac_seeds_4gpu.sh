...
RUN_DIR="${RUN_DIR:-${ROOT}/runs/b2_1_ac_seeds}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${ROOT}/checkpoints/b2_1_ac_seeds}"
MODE="${1:-full}"

names=(A_seed1 A_seed2 C_seed1 C_seed2)
gpus=(2 3 4 5)
...
for i in "${!names[@]}"; do
  ...
  nohup env CUDA_VISIBLE_DEVICES="${gpu}" PYTHONUNBUFFERED=1 \
    "${PYTHON_BIN}" "${ROOT}/experiments/phase0_b21_train.py" \
      --config "${ROOT}/configs/b2_1_ac_seeds/${name}.json" \
      --data-dir "${DATA_DIR}" \
      --output "${output}" \
