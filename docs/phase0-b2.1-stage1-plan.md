...
> 执行状态：2026-09-22 已完成全部六个 Seed 0 实验。预注册主候选 E 未通过继续门槛；完整结果见 [`../results/phase0_b21_stage1/RESULTS.md`](../results/phase0_b21_stage1/RESULTS.md)。以下内容保留为实验前冻结的计划，不按结果反向修改。
...
六个实验 A–F 均已完成训练、离线评估和模型自身连续 rollout。预注册结论为：

- E 相对 A 的组合成功率提高 5.17 个百分点，通过；
- E 的四测试集平均 regret 相对 A **增加** 35.87%，失败；
- E 仅在六类任务中的三类不下降，失败；
- E 明显优于 F，证明完整集合交互有贡献；
- E 的吞吐和显存满足门槛。

因此第一阶段正式决定为 `stop_and_diagnose`，不直接进入以 E 为候选的第二阶段。

事后消融显示 C（完整集合 + Regression + Ranking）表现最好：组合成功率相对 A 提高 9.67 个百分点，四测试集平均 regret 降低 41.93%，六类任务中五类不下降。该结果只用于选择下一轮重新预注册的候选，不改写本轮 E 的失败结论。
