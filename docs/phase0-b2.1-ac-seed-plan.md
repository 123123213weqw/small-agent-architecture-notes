...
> 执行状态：2026-09-22 已完成。A/C Seed 0、1、2 的冻结门槛全部通过；完整结果见 [`../results/phase0_b21_ac_seeds/RESULTS.md`](../results/phase0_b21_ac_seeds/RESULTS.md)。以下第 1–7 节保留为查看 Seed 1、2 结果之前提交的计划。
...
- 组合成功率平均提高 25.5 个百分点；
- 四测试集平均 eviction regret 降低 63.8%；
- 六类组合任务中五类不下降；
- 三个 Seed 的组合成功率和平均 regret 均同时改善；
- C/A 吞吐比为 2.16，显存比为 0.50，参数比为 1.0001。

因此 C 通过 B2.1 多种子确认。该结论只证明当前合成任务上“完整集合交互 + 排序监督”的优化稳定性，不代表语言表示或真实 Reader 已通过。

唯一明确的结构性失败是 `relation_chain`：C 的三种子平均成功率为 0.163，低于 A 的 0.360。进入更大 Reader 前必须单独诊断这种多记录关系链任务。
